...
[
  ...
  # Try bundled old MultiNest first (libnest3 in root directory), then new format
  if test -f "$MULTINEST_DIR/libnest3.a" || test -f "$MULTINEST_DIR/libnest3.so"; then
    MULTINEST_LIBS="$MULTINEST_LIBS -Wl,-rpath,$MULTINEST_DIR -lnest3"
  elif test -f "$MULTINEST_DIR/lib/libmultinest.a" || test -f "$MULTINEST_DIR/lib/libmultinest.so"; then
    MULTINEST_LIBS="$MULTINEST_LIBS -L$MULTINEST_DIR/lib -Wl,-rpath,$MULTINEST_DIR/lib -lmultinest"
  else
    # Fallback to old format in lib/ subdirectory
    MULTINEST_LIBS="$MULTINEST_LIBS -L$MULTINEST_DIR/lib -Wl,-rpath,$MULTINEST_DIR/lib -lnest3"
  fi
