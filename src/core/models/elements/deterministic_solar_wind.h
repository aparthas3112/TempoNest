...
class deterministic_solar_wind_t : public model_element_t {
public:
    ...
    // Apply deterministic solar wind correction to timing residuals
    // Formula: residuals[i] -= (ne_fitted - ne_reference) × tdis2[i]
    void apply(const std::vector<double>& parameter_values, Eigen::VectorXd& residuals) const;
