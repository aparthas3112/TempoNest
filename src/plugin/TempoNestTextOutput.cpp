...
void TNtextOutput(pulsar* psr, int npsr, int ndim, std::string longname, const std::shared_ptr<model_t> model, const std::vector<parameter_stats_t>& stats)
{
    ...
    for (p = 0; p < npsr; p++) {
        ...
        /*	  wrms_pre =
         * sqrt((sumsq_pre/(double)count-sum_pre*sum_pre/pow(double(count),2))/(sumwt/(double)count)/(sumwt/double(count)))*1e6;
         */
        ...
        printf("\n\n");
        printf(
            "PARAMETER          Tempo2-Fit              TempoNest-Fit          Uncertainty   "
            "Difference   Fit\n");
        printf(
            "--------------------------------------------------------------------------------------"
            "-------------\n");
        for (i = 0; i < MAX_PARAMS; i++) {
            for (k = 0; k < psr[p].param[i].aSize; k++) {
                if (psr[p].param[i].fitFlag[k] == 1) {
                    ...
                    /* *180.0/M_PI*60.0*60.0*
                     * 1000.0*SECDAY*365.25/24.0/3600.0*cos(psr[p].param[param_decj].val)); */
        ...
        if (1 == 1) /* Write a new .par file */
        {
            ...
            if (!(fout2 = fopen(fname2, "w"))) {
                            ...
                            if (psr[p].param[i].fitFlag[k] == 1) {
                                ...
                                if (i == param_raj) {
                                    ...
                                    if (psr[p].param[i].err[k] > 1e-12)
                                        ...
                                        fprintf(fout2, " %-25.20Lg", psr[p].param[i].err[k]);
                                }
                            } else {
                                fprintf(fout2, "   ");
                                if (i == param_raj) {
                                    ...
                                    if (psr[p].param[i].prefitErr[k] > 1e-12)
                                        ...
                                        fprintf(fout2, " %-25.20Lg", psr[p].param[i].prefitErr[k]);
                                }
                            }
                            fprintf(fout2, "\n");
                        }
                ...
                if (psr[p].tempo1 == 1) {
                    if (!strcmp(psr[p].clock, "TT(UTC(NIST))"))
                        fprintf(fout2, "%-15.15s%s\n", "CLK", "UTC(NIST)");
                    else if (!strcmp(psr[p].clock, "TT(TAI))"))
                        fprintf(fout2, "%-15.15s%s\n", "CLK", "UTC(BIPM)");
                    else if (!strcmp(psr[p].clock, "TT(UTC(PTB))"))
                        fprintf(fout2, "%-15.15s%s\n", "CLK", "PTB");
                    else if (!strcmp(psr[p].clock, "TT(TA(NIST))"))
                        fprintf(fout2, "%-15.15s%s\n", "CLK", "AT1");
                    else
                        fprintf(fout2, "%-15.15s%s\n", "CLK", psr[p].clock);
                } else
                ...
                /* Add frequency-dependent jumps (FDJUMP) */
                ...
                if (psr[p].param[param_dmmodel].paramSet[0] == 1) {
                    ...
                    if (useDMOFF)
                        for (i = 0; i < psr[p].dmoffsDMnum; i++) {
                            if (psr[p].dmoffsDM_mjd[i] != psr[p].dmoffsCM_mjd[i])
                                useDMOFF = false;
                            break;
                        }
                    if (useDMOFF) {
